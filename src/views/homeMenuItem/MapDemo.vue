<template>
  <div style="height: 100%; width: 100%">
    <div style="padding: 20px; position: relative; z-index: 9999">
      <div class="map-index">
    ...
    <div class="wideContainer">
      <div class="outerContainer">
        ...
        <div class="innerContainerRight">
          <div class="innerContainerRightInnerTop">
            <div class="innerContainerRightInnerTopTable">
              <div class="innerContainerRightInnerTopTableThead">工业设备</div>
              <div class="innerContainerRightInnerTopTableTr">
                <div class="copyTable">正常</div>
                <div class="copyTable">120</div>
              </div>
              <div class="innerContainerRightInnerTopTableTr">
                <div class="copyTable">预警</div>
                <div class="copyTable">21</div>
              </div>
              <div class="innerContainerRightInnerTopTableTr">
                <div class="copyTable">报警</div>
                <div class="copyTable">24</div>
              </div>
              <div class="innerContainerRightInnerTopTableTr">
                <div class="copyTable">离线</div>
                <div class="copyTable">42</div>
              </div>
              <div class="innerContainerRightInnerTopTableTr">
                <div class="copyTable">合计</div>
                <div class="copyTable">42</div>
              </div>
            </div>
            <div class="innerContainerRightInnerTopEchart">
              <div class="systemCoverageBox">
                <p class="systemCoverageBoxTitle">系统覆盖指数</p>
                <p>稳定运行时间:15565天</p>
                <systemCoverageIndex />
              </div>
            </div>
          </div>
          <div class="innerContainerRightInnerBottom">
            <div class="innerContainerRightInnerBottomBody">
              <realTimeAlarmDataRotation />
            </div>
          </div>
        </div>
      </div>
...
<script>
import realTimeAlarmDataRotation from "../components/realTimeAlarmDataRotation";
import systemCoverageIndex from "../components/systemCoverageIndex";
import echarts from "echarts/lib/echarts";
require("@/static/china");

export default {
  name: "generalViewOfTheSite",
  components: { realTimeAlarmDataRotation, systemCoverageIndex },
  data() {
    return {
  ...
  methods: {
    ...
    drawChinaMap(provData) {
      ...
      this.myChart.on("click", (params) => {
        if (params.seriesName === "theMap") {
          ...
          this.changeAreaFlag = true;
          this.nowDeviceName = theName;
          this.mapIndex.push({
            name: theName,
            code: theName,
          });
    ...
    drawProvinceMap(provinceName, cityData) {
      ...
      this.myChart.on("click", (params) => {
        // 先确认是否有地图，有地图继续请求
        if (
          this.areaMapping[provinceName].children &&
          this.areaMapping[provinceName].children[params.name] &&
          this.areaMapping[provinceName].children[params.name].code !==
            undefined
        ) {
          this.drawCityMap(
            provinceCode,
    ...
    drawCityMap(provinceCode, cityCode, areaData, cityName) {
      ...
      this.myChart = echarts.init(document.getElementById("chinaMap"));
      // 这里更换市级地图json
      const url = require(`@/static/map/province/${provinceCode}/${cityCode}.json`);
      // 获取json
    ...
    changeMapIndexCode(index) {
      ...
      switch (index) {
        ...
        case 1:
          this.drawCityMap(
            this.mapIndex[0].code,
            this.mapIndex[1].code,
            [],
            this.mapIndex[1].name
          );
          break;
        default:
